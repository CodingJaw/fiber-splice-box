...
`cr6/filament_spool_adapterV3.scad` keeps V2 intact and adds printable
dovetail keys/sockets to hold the two halves together. The defaults adapt a
30 mm shaft to a 52 mm spool bore with a 60 mm outer keeper/flange supported
against the side of the spool.

Adjust the shaft, spool, keeper, straight spool-seat width, taper end diameter,
dovetail, overall width, and clearance variables at the top of the latest file.
Use new versioned SCAD filenames for future adapter changes
...
Render the latest V3 from the command line:

```sh
openscad -o cr6/filament_spool_adapterV3.stl cr6/filament_spool_adapterV3.scad
```
