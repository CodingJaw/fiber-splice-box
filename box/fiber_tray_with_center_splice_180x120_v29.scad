/*
  Fiber Tray With Centered MID-FLEX Oval Fiber Holder + Straight 4-Splice Holder 180x120 V29
...
// Radial inset from the main racetrack centerline to the second inner racetrack centerline.
Second_Inner_Racetrack_Inset = 18.0;   // [8:0.5:35]
...
function second_inner_racetrack_radius(r) = max(3, r - Second_Inner_Racetrack_Inset);

module second_inner_racetrack_holder(r, d, gap, wall_t, wall_h) {
    inner_r = second_inner_racetrack_radius(r);

    inner_racetrack_side_walls(inner_r, d, gap, wall_t, wall_h);

    if (Second_Inner_Racetrack_Fingers)
        racetrack_fingers(inner_r, d, gap, wall_t, wall_h);
}

module inner_racetrack_side_walls(r, d, gap, wall_t, wall_h) {
    // The second inner racetrack is a closed loop: no off-ramp openings and no short-end inner-wall removals.
...
module second_inner_racetrack_mock_fiber_path() {
    racetrack_cyl_path(second_inner_racetrack_radius(path_radius()), racetrack_center_distance(), Mock_Fiber_Diameter, fiber_center_z_local());
}

module second_inner_racetrack_keepout_path() {
    racetrack_cyl_path(second_inner_racetrack_radius(path_radius()), racetrack_center_distance(), fiber_keepout_d(), fiber_center_z_local());
}
