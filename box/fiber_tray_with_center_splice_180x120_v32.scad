/*
  Fiber Tray With Centered MID-FLEX Oval Fiber Holder + Straight 4-Splice Holder 180x120 V32
  ...
  Default layout:
    ...
    - 180x120 V27 sets the tray hinge center access gap to 61 mm so the tray-side
      hinge ears sit outside the box-side hinge barrels.
    - 180x120 V28 widens the hinge bridge/rib to match the wider 61 mm center-gap hinge span.
    - 180x120 V29 adds an optional second inner racetrack while leaving the center splice holder unchanged.
    - 180x120 V30 reworks the second racetrack so it shares the first racetrack inner wall.
    - 180x120 V31 lowers OpenSCAD CSG complexity by making the second inner racetrack
      a simple shared-wall lane by default, without extra fingers/keepout cuts,
      and by disabling the expensive global fiber keepout by default.
    - 180x120 V32 doubles the on/off ramps and opens the shared-wall racetrack ends
      so there are paths between ramps, racetracks, and the center splice rack area.
...
/* [Second Inner Racetrack] */

// Adds a second, smaller racetrack inside the main oval.
Show_Second_Inner_Racetrack = true;    // [true,false]
// Extra clearance between the shared wall and the second channel centerline.
// Leave at 0 for a true shared-wall racetrack using the current channel gap and wall thickness.
Second_Inner_Racetrack_Extra_Inset = 0.0; // [0:0.25:8]
// Adds keeper fingers to the second inner racetrack using the same finger profile.
Second_Inner_Racetrack_Fingers = false; // [false,true]
// Applies the same fiber keepout cut to the second inner racetrack.
Second_Inner_Racetrack_Keepout = false; // [false,true]
// Opens the rounded ends of the second/shared racetrack so fiber can move between lanes and into the splice rack area.
Second_Inner_Racetrack_Open_Short_Ends = true; // [true,false]
...
Off_Ramp_Mode = 3;                     // [0:Off, 1:Top long side only, 2:Bottom long side only, 3:Both long sides]
...
Hinge_Center_Access_Gap = 61.0;         // [5:0.5:80]
...
Hinge_Wall_Bridge_Width = 105.0;         // [20:1:130]
...
Apply_Global_Fiber_Keepout = false;    // [false,true]
...
// If an export shows "Normalized tree is growing past 200000 elements", keep the second
// inner racetrack fingers/keepout disabled and leave Apply_Global_Fiber_Keepout off.
// Re-enable keepouts only for final clearance checking when needed.
CSG_Complexity_Note = "Keepouts off for faster export"; // [Keepouts off for faster export]
...
module tray_with_centered_mid_flex_fiber_holder() {
    difference() {
        ...
        if (Show_Second_Inner_Racetrack && Second_Inner_Racetrack_Keepout)
            translate([Holder_X_Offset, Holder_Y_Offset, holder_mount_z()])
                rotate([0, 0, Holder_Rotation])
                    second_inner_racetrack_keepout_path();
    ...
    if (Show_Mock_Fiber && Show_Second_Inner_Racetrack)
        translate([Holder_X_Offset, Holder_Y_Offset, holder_mount_z()])
            rotate([0, 0, Holder_Rotation])
                %second_inner_racetrack_mock_fiber_path();
...
module oval_channel_holder() {
    ...
    union() {
        ...
        if (Show_Second_Inner_Racetrack)
            second_inner_racetrack_holder(r, d, gap, wall_t, wall_h);
...
function second_inner_racetrack_radius(r, gap, wall_t) =
    max(3, r - gap - wall_t - Second_Inner_Racetrack_Extra_Inset);

module second_inner_racetrack_holder(r, d, gap, wall_t, wall_h) {
    inner_r = second_inner_racetrack_radius(r, gap, wall_t);

    shared_wall_second_racetrack_side_walls(inner_r, d, gap, wall_t, wall_h);

    if (Second_Inner_Racetrack_Fingers)
        racetrack_fingers(inner_r, d, gap, wall_t, wall_h);
}

module shared_wall_second_racetrack_side_walls(r, d, gap, wall_t, wall_h) {
    // The second racetrack centerline is offset inward by one channel pitch
    // (gap + wall thickness), so its outer wall overlaps/shares the first
    // racetrack inner wall. The inner wall below creates the new second lane.
    ...
    for (a = [90 : -Arc_Angle_Step : -90 + 0.001]) {
        for (side = [-1, 1]) {
            if (!Second_Inner_Racetrack_Open_Short_Ends) {
...
module second_inner_racetrack_mock_fiber_path() {
    racetrack_cyl_path(second_inner_racetrack_radius(path_radius(), effective_slot_gap(), selected_wall_thickness()), racetrack_center_distance(), Mock_Fiber_Diameter, fiber_center_z_local());
}

module second_inner_racetrack_keepout_path() {
    racetrack_cyl_path(second_inner_racetrack_radius(path_radius(), effective_slot_gap(), selected_wall_thickness()), racetrack_center_distance(), fiber_keepout_d(), fiber_center_z_local());
}
