/*
  Fiber box base - SC holder centered - stacked print layout - 180x120 V24
...
// CutFromTop is the lid depth, so the hinge is limited to the bottom-box opening at CaseHeight - CutFromTop.
BlueHingeHeightPreset     = "Touch bottom-box top"; // [Lower - 6mm, Lower - 3mm, Touch bottom-box top, Custom numeric offset]
// Used only when BlueHingeHeightPreset is Custom numeric offset. Positive raises the hinge; negative lowers it.
// The final hinge position is clamped so the hinge stays below the lid split.
...
// CutFromTop is the lid depth; the bottom box only reaches CaseHeight - CutFromTop.
// Limit the hinge so the top of the barrel/wedge can reach that split line without going into the lid.
blue_bottom_box_top_z    = CaseHeight - CutFromTop;
blue_axis_z_requested    = blue_bottom_box_top_z - blue_knuckle_od/2 + blue_hinge_z_offset;
blue_axis_z_max          = blue_bottom_box_top_z - blue_knuckle_od/2;
blue_axis_z_min          = BottomTopThickness + blue_knuckle_od/2;
blue_axis_z              = min(max(blue_axis_z_requested, blue_axis_z_min), blue_axis_z_max);
...
echo(str("Blue hinge spacing preset: ", BlueHingeSpacingPreset,
         ...
         "; clamped hinge axis Z = ", blue_axis_z, " mm",
         "; bottom-box top Z = ", blue_bottom_box_top_z, " mm"));
