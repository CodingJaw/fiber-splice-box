...
// Two lower-side cable entry holes. 1/2 inch = 12.7 mm.
...
//===============================================================================
// This version intentionally makes no SC openings through the enclosure wall.
// Exterior openings are limited to the two 1/2 inch cable entry holes on the lower side.

module FiberBoxBottomAdditions(){
    floor_z = BottomTopThickness;
    inner_w = CaseWidth - 2*SideWallThickness;

    // Screw-down style cable strain reliefs immediately inside the two bottom cable holes.
    for (x=[-FiberBottomHoleSpacing/2, FiberBottomHoleSpacing/2]){
        translate([x, -CaseWidth/2 + SideWallThickness + 11, floor_z])
            FiberCableClamp();
    }

    // Raised horizontal SC adapter holder near the lower/back side of the box.
    // SC adapters drop through these two elevated rectangular pockets and are plugged from above and below inside the box.
    translate([0, -CaseWidth/2 + SideWallThickness + SC_Holder_Y_Offset, floor_z])
        SCInternalElevatedHolder();
}

module FiberBoxBottomCuts(){
    // Only exterior cuts added by the fiber-box modification:
    // two 1/2 inch cable entry holes through the lower side wall.
    for (x=[-FiberBottomHoleSpacing/2, FiberBottomHoleSpacing/2]){
        translate([x, -CaseWidth/2 + SideWallThickness/2, BottomTopThickness + FiberBottomHoleCenterZ])
            rotate([90,0,0])
                cylinder(h=SideWallThickness+1.2, d=FiberBottomHoleDiameter, center=true);
    }
}

module FiberCableClamp(){
    base_t = 3;
    clamp_l = 24;
    clamp_w = 16;
    bridge_h = FiberCableSheathDiameter + 4;

    difference(){
        union(){
            // Floor pad bonded to inside bottom of case.
            translate([0,0,base_t/2]) roundedBox([clamp_l, clamp_w, base_t], 2, 1);
            // Raised screw bosses on each side of the cable.
            for (x=[-FiberClampScrewSpacing/2, FiberClampScrewSpacing/2]){
                translate([x,0,base_t]) cylinder(h=bridge_h, d=7, center=false);
            }
            // Cross bridge that clamps over the 5 mm cable sheath path.
            translate([0,0,base_t + bridge_h - 1.5]) roundedBox([clamp_l, clamp_w, 3], 1.5, 1);
        }
        // Cable channel, aligned with the cable entering through the side wall.
        translate([0,0,base_t + FiberCableSheathDiameter/2 + 0.8])
            rotate([90,0,0]) cylinder(h=clamp_w+1.0, d=FiberCableSheathDiameter+0.8, center=true);
        // Pilot holes for clamp screws. These are internal printed holes only.
        for (x=[-FiberClampScrewSpacing/2, FiberClampScrewSpacing/2]){
            translate([x,0,base_t-0.1]) cylinder(h=bridge_h+0.4, d=FiberClampScrewDiameter, center=false);
        }
    }
}

module SCInternalElevatedHolder(){
    // Image-style SC coupler rack:
    // - no exterior SC holes
    // - two internal rectangular tunnels for simplex SC couplers
    // - couplers slide through the vertical bracket front-to-back inside the box
    // - bracket is raised off the floor so patch leads can plug from both sides inside the box
    plate_l = max(55, SC_Holder_Spacing + SC_Cutout_Width + 22);
    base_w  = 34;
    base_h  = SC_Holder_Base_Height;
    block_h = SC_Holder_Block_Height;
    block_t = SC_Holder_Block_Thickness;
    cut_z   = base_h + 9.5;

    union(){
        // Raised base platform bonded to the inside floor.
        translate([0,0,base_h/2])
            roundedBox([plate_l, base_w, base_h], 2, 1);

        // Vertical retaining bracket, similar to the bracket shown in the reference image.
        // The rectangular SC openings pass through this bracket in the front/back direction.
        translate([0,0,base_h + block_h/2])
            difference(){
                roundedBox([plate_l, block_t, block_h], 1.5, 1);

                for (x=[-SC_Holder_Spacing/2, SC_Holder_Spacing/2]){
                    // Main SC adapter body pass-through. Axis is Y, so the adapter faces front/back inside the box.
                    translate([x,0,cut_z - (base_h + block_h/2)])
                        cube([SC_Cutout_Width, block_t + 1.2, SC_Cutout_Height], center=true);

                    // Top latch relief for common SC simplex couplers.
                    translate([x,0,cut_z - (base_h + block_h/2) + SC_Cutout_Height/2 + 1.1])
                        cube([SC_Cutout_Width + 1.2, block_t + 1.4, 2.2], center=true);
                }
            }

        // Two small keeper lips on the front and rear faces of the bracket.
        // These help keep clip-in SC couplers from rocking while still allowing test fitting/filing.
        for (y=[-block_t/2-1.0, block_t/2+1.0]){
            translate([0,y,base_h + block_h/2])
                difference(){
                    roundedBox([plate_l, 2.0, block_h], 0.8, 1);
                    for (x=[-SC_Holder_Spacing/2, SC_Holder_Spacing/2]){
                        translate([x,0,cut_z - (base_h + block_h/2)])
                            cube([SC_Cutout_Width + 2.0, 2.4, SC_Cutout_Height + 2.4], center=true);
                    }
                }
        }

        // Optional internal screw holes/pads in the base for adding a printed or metal clamp bar later.
        for (x=[-plate_l/2+8, plate_l/2-8]){
            translate([x, base_w/2-7, base_h])
                difference(){
                    cylinder(h=5, d=8, center=false);
                    translate([0,0,-0.1]) cylinder(h=5.3, d=SC_Holder_ScrewDiameter, center=false);
                }
        }
    }
}
