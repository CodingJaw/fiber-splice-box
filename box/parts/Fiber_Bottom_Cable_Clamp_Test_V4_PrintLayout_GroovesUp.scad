/*
  Fiber Bottom Cable Clamp Test - V4 Removable Split Bushing Clamp - Print Layout Grooves Up
...
module print_layout_grooves_up(){
    ...
    for (i = [0:len(Cable_Ys)-1]) {
        ypos = (i - (len(Cable_Ys)-1)/2) * part_pitch_y;

        // Lower clamp half: bottom is flat on Z=0, cable saddle faces up.
        translate([0, ypos, -Lower_Z0])
        ...
        translate([0, ypos - row_gap_y, Upper_Z1])
            rotate([180,0,0])
