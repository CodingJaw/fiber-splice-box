/*
  Fiber Bottom Cable Clamp Test - V5 Removable Split Bushing Clamp - Fixed Print Layout
  ...
  - Print layout places both upper and lower halves flat on the XY build plane with the cable grooves facing up.
...
// -----------------------------
Show_Mock_Wall      = false;   // normally false for print layout; turn on only for assembled checking
Show_Mock_Cables    = false;   // normally false for print layout; turn on only for assembled checking
Show_Assembled      = false;   // true = assembled/alignment preview, false = print layout loose parts
Explode_Preview     = false;    // raises upper halves for visual checking when Show_Assembled=true
Show_Only_One_Clamp = false;    // false = four print pieces for two clamps; true = two print pieces for one clamp
...
if (Show_Assembled) {
    ...
    // Print layout: loose parts on the XY build plane.
    // Lower halves keep their assembly orientation because their cable grooves already face upward.
    // Upper halves are flipped so their cable grooves also face upward.
    // This avoids roof bridging across the cable groove and makes the preview match a practical slicer layout.
    print_layout_grooves_up();
}
...
module print_layout_grooves_up(){
    // Fixed V5 print layout:
    // - Each cable clamp produces two separate printable parts.
    // - For two clamps, this creates four independent pieces in a clean 2 x 2 grid.
    // - Parts are separated along X by column pitch and along Y by row pitch.
    // - Both lower and upper halves sit flat on the XY build plate with grooves facing upward.
    part_pitch_x = 38;
    row_y_lower  = 18;
    row_y_upper  = -18;

    for (i = [0:len(Cable_Ys)-1]) {
        xpos = (i - (len(Cable_Ys)-1)/2) * part_pitch_x;

        // Lower clamp half: bottom is flat on Z=0, cable saddle faces up.
        // Shift by -Total_Center_X so the part is visually centered in its column.
        translate([xpos - Total_Center_X, row_y_lower, -Lower_Z0])
            removable_split_clamp_lower(0);

        // Upper clamp half: flipped for printing so its cable groove faces up.
        // After printing, flip this part back over for assembly.
        translate([xpos - Total_Center_X, row_y_upper, Upper_Z1])
            rotate([180,0,0])
                removable_split_clamp_upper(0);
    }
}
