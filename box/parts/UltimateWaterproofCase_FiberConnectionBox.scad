...
CountOfSideWallHoles     = 2;     //[1:1:3]
// Diameter of the holes
SideWallHoleDiameter     = 12.0;  //[1:0.1:80]
// Add or decrease height position
SiedWallHoleOffset_Z     = 0;     


/* [Fiber connection box additions] */
// Adds an internal hinged fiber management tray, two SC connector holders, and fiber entry holes
EnableFiberConnectionBox  = true;
// Angle of the swing-out splice/wrap plate for viewing/fit checking. 0 = folded in, 65 = swung out.
FiberPlateOpenAngle       = 65;     // [0:5:120]
// Two SC simplex adapter openings. Adjust if your specific coupler datasheet differs.
SC_Cutout_Width           = 10.5;
SC_Cutout_Height          = 13.5;
SC_Holder_Spacing         = 28;
// Fiber cable pass-through holes on the side wall
FiberEntryHoleDiameter    = 8.0;
FiberEntryHoleSpacing     = 24;
// Hinge pin clearance diameter. Use a small screw, filament, or metal pin.
FiberHingePinDiameter     = 2.4;
...
//===============================================================================
//                      F I B E R   C O N N E C T I O N   B O X
//===============================================================================
// Added for SC fiber termination / slack management.
// The SC rectangular openings are parametric because SC adapter brands vary slightly.

module FiberBoxBottomAdditions(){
    bottom_h = CaseHeight-CutFromTop;
    inner_l = Caselength-2*SideWallThickness;
    inner_w = CaseWidth-2*SideWallThickness;
    floor_z = BottomTopThickness;

    // Two SC adapter holder frames on the front wall.
    for (x=[-SC_Holder_Spacing/2, SC_Holder_Spacing/2]){
        translate([x, -CaseWidth/2+SideWallThickness+2.2, floor_z+SC_Cutout_Height/2+4])
            SCHolderFrame();
    }

    // Internal hinge fixed to left side of case.
    translate([-inner_l/2+5, 0, floor_z+2]) FiberFixedHinge();

    // Swing-out fiber management plate. Shown open for clearance checking.
    translate([-inner_l/2+5, 0, floor_z+2])
        rotate([0,0,FiberPlateOpenAngle])
            FiberSwingPlate();

    // Small tie-down loops / bridge anchors on floor for incoming and outgoing fiber.
    for (x=[-35,35]){
        translate([x, CaseWidth/2-SideWallThickness-10, floor_z+1.25]) FiberTieBridge();
    }
}
...
module FiberBoxBottomCuts(){
    ...
    // Rectangular panel openings for two SC simplex couplers through the front wall.
    for (x=[-SC_Holder_Spacing/2, SC_Holder_Spacing/2]){
        translate([x, -CaseWidth/2+SideWallThickness/2, floor_z+SC_Cutout_Height/2+4])
            cube([SC_Cutout_Width, SideWallThickness+0.8, SC_Cutout_Height], center=true);
    }

    // Two round fiber entry/exit holes through the right side wall.
    for (y=[-FiberEntryHoleSpacing/2, FiberEntryHoleSpacing/2]){
        translate([CaseLengthSafe()/2-SideWallThickness/2, y, floor_z+10])
            rotate([0,90,0]) cylinder(h=SideWallThickness+1.2, d=FiberEntryHoleDiameter, center=true);
    }

    // Hinge pin clearance through fixed hinge barrels and tray barrels.
    translate([-(Caselength-2*SideWallThickness)/2+5, 0, floor_z+2])
        cylinder(h=26, d=FiberHingePinDiameter, center=false);
}

function CaseLengthSafe() = Caselength;

module SCHolderFrame(){
    // Printed retaining frame around the wall cutout. Sized for common SC simplex panel adapters.
    frame_w = SC_Cutout_Width + 8;
    frame_h = SC_Cutout_Height + 8;
    frame_d = 6;
    difference(){
        union(){
            color("LightGreen") roundedBox([frame_w, frame_d, frame_h], 1.2, 1);
            // small stop ledges for a snap-in SC coupler shoulder
            translate([0,2.8, frame_h/2-1.2]) cube([frame_w,2.0,2.4], center=true);
            translate([0,2.8,-frame_h/2+1.2]) cube([frame_w,2.0,2.4], center=true);
        }
        cube([SC_Cutout_Width, frame_d+1.0, SC_Cutout_Height], center=true);
        // relief notches for plastic latch ears, if present
        translate([0,0, SC_Cutout_Height/2+1.3]) cube([SC_Cutout_Width+2, frame_d+1.2, 1.8], center=true);
        translate([0,0,-SC_Cutout_Height/2-1.3]) cube([SC_Cutout_Width+2, frame_d+1.2, 1.8], center=true);
    }
}

module FiberFixedHinge(){
    // Three fixed hinge barrels mounted to the enclosure floor/side.
    color("SteelBlue") union(){
        for (z=[0,10,20]){
            translate([0,0,z]) cylinder(h=6, d=7, center=false);
        }
        translate([0,0,13]) cube([8,14,26], center=true);
    }
}

module FiberSwingPlate(){
    plate_l = 70;
    plate_w = 54;
    plate_t = 3;
    // hinge leaves that interlock with the fixed hinge barrels
    color("Gold") union(){
        for (z=[6,16]){
            translate([0,0,z]) cylinder(h=4, d=6.4, center=false);
        }
        translate([plate_l/2+3,0,1.5]) difference(){
            roundedBox([plate_l, plate_w, plate_t], 4, 1);
            // lightening slots / zip tie slots
            for (y=[-16,16]) translate([5,y,0]) roundedBox([38,4,plate_t+0.4],1.8,1);
        }
        // Fiber wrap posts/spools on the tray. Keep bends gentle; adjust size if needed.
        for (x=[30,56]){
            translate([x,0,plate_t]) FiberWrapPost();
        }
        // Raised rim at the free end to help retain loops.
        translate([plate_l+4,0,plate_t+2]) roundedBox([3,plate_w-8,4],1.5,1);
    }
}

module FiberWrapPost(){
    difference(){
        union(){
            cylinder(h=9, d=18, center=false);
            translate([0,0,9]) cylinder(h=2, d=24, center=false);
        }
        translate([0,0,-0.2]) cylinder(h=12, d=8, center=false);
    }
}

module FiberTieBridge(){
    difference(){
        roundedBox([18,7,3],1.5,1);
        translate([0,0,0]) roundedBox([11,3.2,3.5],1.0,1);
    }
}
