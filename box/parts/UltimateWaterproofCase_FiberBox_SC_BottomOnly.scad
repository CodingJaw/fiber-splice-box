...
SC_Cutout_Depth           = 13.5;
SC_Holder_Plate_Height    = 17;
SC_Holder_Plate_Thickness = 4;
SC_Holder_Y_Offset        = 32;
...
module SCInternalElevatedHolder(){
    plate_l = max(70, SC_Holder_Spacing + SC_Cutout_Width + 24);
    plate_w = 28;
    leg_h = SC_Holder_Plate_Height;
    plate_t = SC_Holder_Plate_Thickness;

    union(){
        // Four support legs keep the SC adapter plate elevated off the floor.
        for (x=[-plate_l/2+6, plate_l/2-6], y=[-plate_w/2+5, plate_w/2-5]){
            translate([x,y,0]) cylinder(h=leg_h, d=7, center=false);
        }
        // Small rear stiffener wall.
        translate([0, plate_w/2-2, leg_h/2]) roundedBox([plate_l, 4, leg_h], 1.2, 1);

        // Horizontal SC holder plate with two vertical rectangular adapter pockets.
        translate([0,0,leg_h])
            difference(){
                roundedBox([plate_l, plate_w, plate_t], 2, 1);
                for (x=[-SC_Holder_Spacing/2, SC_Holder_Spacing/2]){
                    translate([x,0,0]) cube([SC_Cutout_Width, SC_Cutout_Depth, plate_t+1.0], center=true);
                    // Small latch reliefs front/back for common simplex SC coupler ears.
                    translate([x, SC_Cutout_Depth/2+1.3, 0]) cube([SC_Cutout_Width+2, 2, plate_t+1.2], center=true);
                    translate([x,-SC_Cutout_Depth/2-1.3, 0]) cube([SC_Cutout_Width+2, 2, plate_t+1.2], center=true);
                }
            }

        // Raised labels/rims around each SC pocket to help locate the adapters.
        for (x=[-SC_Holder_Spacing/2, SC_Holder_Spacing/2]){
            translate([x,0,leg_h + plate_t + 0.75])
                difference(){
                    roundedBox([SC_Cutout_Width+8, SC_Cutout_Depth+8, 1.5], 1.2, 1);
                    cube([SC_Cutout_Width+1, SC_Cutout_Depth+1, 2], center=true);
                }
        }
    }
}
