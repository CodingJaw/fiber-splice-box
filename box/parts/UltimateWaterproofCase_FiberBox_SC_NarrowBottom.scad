...
module FiberBoxBottomAdditions(){
    ...
    // Raised internal SC adapter holder near the same narrow end as the cable entries.
    // Rotated with the cable entries so the connector tunnels run front/back along X inside the box.
    translate([-Caselength/2 + SideWallThickness + SC_Holder_End_Offset, 0, floor_z])
        rotate([0,0,90])
