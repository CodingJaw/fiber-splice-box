/*
  SC/APC Simplex Coupler Holder - V8 Drawing-Based Fit + No Center Screw Collision

  What V8 fixes:
  - Screw posts are never placed on a connector centerline.
  - One connector now gets two end screws only; two connectors get left/gap/right screws.
  - Odd holder counts place intermediate screws in the gaps between connectors, not through a connector.
  - Mock coupler dimensions were corrected from the supplied drawing:
      body/opening width: 9.3 mm
      body/opening height: 12.8 mm
      overall coupler length: 27.5 mm
      flange thickness: 2.5 mm
      flange/body height reference: 14.8 mm
  - Holder flange slots are widened and the center opening height is reduced.
...
SC_Flange_W = 24.6;         // total flange width across both ears; widened for slide-in flange slot
SC_Flange_H = 14.8;         // flange height top-to-bottom from supplied drawing
SC_Flange_T = 2.5;          // flange thickness along plug direction from supplied drawing
SC_Body_W = 9.3;            // central connector body/opening width from supplied drawing
SC_Body_H = 12.8;           // central connector body/opening height from supplied drawing
SC_Body_Depth_Y = 27.5;     // central connector body length from supplied drawing

SC_Body_Clear_W = 10.1;     // open center width in holder: 9.3mm body + print clearance
SC_Body_Clear_H = 13.6;     // open center height/ghost: reduced because old opening was too tall
SC_Clearance = 0.40;
...
module mock_sc_coupler() {
    // Drawing-based simplified mock:
    // - center body/port is 9.3 W x 12.8 H x 27.5 L
    // - flange plate is 2.5 mm thick and 14.8 mm high
    // - full flange width remains adjustable because the drawing does not clearly show ear-to-ear width
    zc = flange_bottom_z + SC_Flange_H/2;

    color(Mock_Color, Mock_Alpha) {
        // Full connector body through the holder.
        translate([0,0,zc])
            cube([SC_Body_W, SC_Body_Depth_Y, SC_Body_H], center=true);

        // Thin flange/ear plate that slides down into the side rails.
        translate([0,0,zc])
            cube([SC_Flange_W, SC_Flange_T, SC_Flange_H], center=true);

        // Slight front/back collars matching the 9.3 x 12.8 face size.
    ...
    // Front/back optical bore marks for visual checking only.
...
module screw_positions(count=count_active) {
    ...
    // End screws. These are outside the connector slots.
    for (x=[left_x, right_x])
        translate([x,0,0]) children();

    // Intermediate screws are placed only in gaps between adjacent connectors.
    // This avoids a screw post through the middle connector on odd holder counts.
    if (count > 1)
        for (i=[0:count-2]) {
