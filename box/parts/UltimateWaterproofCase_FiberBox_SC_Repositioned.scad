...
module FiberBoxBottomAdditions(){
    ...
    // Screw-down style cable strain reliefs immediately inside the two 1/2 inch holes.
    // The entries are now on the narrow end of the box, so the cable path runs along X.
    for (y=[-FiberBottomHoleSpacing/2, FiberBottomHoleSpacing/2]){
        translate([-Caselength/2 + SideWallThickness + 11, y, floor_z])
            rotate([0,0,90])
                FiberCableClamp();
    }

    // Raised internal SC adapter holder moved just past the box halfway point.
    // It is also shifted toward the back side to provide clearance for a future swing-out tray.
    // Rotated with the cable entries so the connector tunnels run front/back along X inside the box.
    translate([-Caselength/2 + SideWallThickness + SC_Holder_End_Offset, SC_Holder_Back_Offset, floor_z])
        rotate([0,0,90])
...
module FiberBoxBottomCuts(){
    ...
    // two 1/2 inch cable entry holes through the narrow end wall.
    for (y=[-FiberBottomHoleSpacing/2, FiberBottomHoleSpacing/2]){
