...
// Select output
// -----------------------------
part = "print_layout";    // "print_layout", "assembly", "base", "lid", "mock"
show_mocks = false;
...
flange_t = 2.85;           // flange thickness along X - corrected
...
// Print-ready layout
// -----------------------------
// Places the base and lid as separate printable parts on the XY plane.
//
// Important fix:
// The assembly lid has very small underside pressure ribs that extend below
// the lid's local Z=0 by lid_press_rib_drop. If exported directly, slicers
// can report the model as floating or not fully on the build plate.
//
// For printing only, the lid is flipped so the large flat outside face sits
// directly on the XY plane. After printing, physically flip the lid over for
// assembly so the ribs face downward toward the SC adapters.
module printable_lid_on_bed() {
    translate([0, 0, lid_t])
        rotate([180, 0, 0])
            top_lid();
}
...
module print_layout() {
    // Base sits directly on XY plane.
    holder_base();

    // Lid sits directly on XY plane, outside face down, ribs facing upward.
    translate([base_l + 8, 0, 0])
        printable_lid_on_bed();
}
