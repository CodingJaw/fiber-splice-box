...
//
// V11 adds right-hinge relief inner barrel-edge rounding controls while preserving inverted relief placement.
...
right_base_relief_round_inner_barrel_corners = true;
right_base_relief_inner_barrel_corner_radius = 4.0;
...
module right_base_relief_2d(length, depth, inner_side, corner_radius, square_amount, round_inner_barrel_corner, inner_barrel_corner_radius) {
    if (round_inner_barrel_corner) {
        difference() {
            base_relief_2d(length, depth, 1, corner_radius, square_amount);
            translate([inner_side < 0 ? 0 : length, 0])
                circle(r = max(0, inner_barrel_corner_radius));
        }
    } else {
        base_relief_2d(length, depth, 1, corner_radius, square_amount);
    }
...
module right_base_relief_cut(x_start, inner_side, length, depth, corner_radius, square_amount, thickness) {
    translate([x_start, barrel_center_y, -barrel_outer_diameter / 2 - epsilon])
        linear_extrude(height = thickness + 2 * epsilon)
            right_base_relief_2d(
                length,
                depth,
                inner_side,
                corner_radius,
                square_amount,
                right_base_relief_round_inner_barrel_corners,
                right_base_relief_inner_barrel_corner_radius
            );
}
        ...
        union() {
            difference() {
                ...
                if (right_base_relief_enabled) {
                    right_base_relief_cut(-barrel_total_length / 2, 1, left_outer_barrel_length, right_base_relief_depth, right_base_relief_corner_radius, right_base_relief_square_amount, right_leaf_thickness);
                    right_base_relief_cut(barrel_total_length / 2 - left_outer_barrel_length, -1, left_outer_barrel_length, right_base_relief_depth, right_base_relief_corner_radius, right_base_relief_square_amount, right_leaf_thickness);
                }
            }
