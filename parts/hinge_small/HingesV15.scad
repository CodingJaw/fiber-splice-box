...
//
// V15 flips the shaft-up spacer view so the larger flat head sits on the build plate.
...
spacer_nut_trap_enabled = false;
spacer_nut_across_flats = 5.5;
spacer_nut_thickness = 2.4;
spacer_nut_embed_depth = 1.5;
spacer_nut_clearance = 0.2;
...
function hex_circumscribed_diameter(across_flats) =
...
module spacer_nut_trap() {
    translate([0, 0, spacer_nut_embed_depth])
        rotate([0, 0, 30])
            cylinder(
                h = spacer_nut_thickness + epsilon,
                d = hex_circumscribed_diameter(spacer_nut_across_flats + spacer_nut_clearance),
                $fn = 6
            );
}
...
module spacer_object_shaft_up() {
    translate([0, 0, spacer_shaft_length + spacer_head_thickness])
        rotate([180, 0, 0])
            spacer_core();
}
