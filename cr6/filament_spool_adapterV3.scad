// Parametric two-piece filament spool adapter V3.
// Dimensions are in millimeters.
//
// V3 default values match the existing CR-6 STL intent:
// - 30 mm printer shaft
// - 52 mm spool bore
...
// V3 dovetails: the left half gets two male keys and the right half gets
// matching sockets. They run axially through the straight spool-seat area so
// the halves slide together from the side and resist pulling apart.
dovetail_length = 3.2;     // How far each key reaches across the split.
dovetail_neck = 3.0;       // Narrow side of the dovetail at the split face.
dovetail_head = 5.2;       // Wider locking side of the dovetail.
dovetail_clearance = 0.25; // Extra space in the socket for printable fit.
dovetail_y_offset = 22;    // Distance from center to upper/lower dovetails.
...
module dovetail_2d(extra = 0) {
    // Trapezoid points from the split face toward the mating half.
    polygon([
        [0, -(dovetail_neck + extra) / 2],
        [dovetail_length + extra, -(dovetail_head + extra) / 2],
        [dovetail_length + extra, (dovetail_head + extra) / 2],
        [0, (dovetail_neck + extra) / 2]
    ]);
}

module dovetail_pair_2d(extra = 0) {
    for (y = [-dovetail_y_offset, dovetail_y_offset])
        translate([-split_gap / 2, y])
            dovetail_2d(extra);
}

module dovetail_keys(height) {
    linear_extrude(height = height)
        dovetail_pair_2d();
}

module dovetail_sockets(height) {
    translate([0, 0, -0.1])
        linear_extrude(height = height + 0.2)
            dovetail_pair_2d(dovetail_clearance);
}
...
module adapter_half(side = 1) {
    ...
    difference() {
        union() {
            ...
            // Male dovetails live on one half only; the mating half has sockets.
            if (side < 0)
                dovetail_keys(seat_width);
        }

        // The hull used for the taper can bridge across the center bore.
        // Subtract the shaft bore through the finished half as a final step so
        // the 30 mm shaft hole always passes through the entire adapter width.
        translate([0, 0, -0.1])
            linear_extrude(height = adapter_width + 0.2)
                circle(d = shaft_bore);

        if (side > 0)
            dovetail_sockets(seat_width);
    }
}
