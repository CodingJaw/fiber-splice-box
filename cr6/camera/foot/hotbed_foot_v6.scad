...
// The notch-facing side is labeled Notch Front; added material there follows the notch shape.
...
// Which outside walls get extra material. Notch Front follows the original notch-shaped geometry.
wall_expansion_area = "Sides and Notch Front"; // [Sides and Notch Front, Sides Only, Notch Front Only, All Outside]
...
module outside_wall_reinforcement(length, width, height, expansion, area) {
  ...
  if (expansion > 0) {
    union() {
      if (area == "Sides and Notch Front" || area == "Sides Only" || area == "All Outside") {
      ...
      // All Outside is the only mode that adds a plain rear wall away from the notch.
      if (area == "All Outside") {
...
module notch_front_reinforcement(expansion, notch_side, notch_width_scale_value, notch_depth_scale_value) {
  if (expansion > 0) {
    mirror_for_notch = notch_side == "Mirrored Right" ? [-1, 1, 1] : [1, 1, 1];

    // The notch/front side of this mesh is +Y. A shifted copy thickens that side
    // while preserving the large notch and the smaller top notch cut profiles.
    translate([0, expansion, 0])
      scale(mirror_for_notch)
        scale([notch_width_scale_value, notch_depth_scale_value, 1])
          hotbed_foot_original_mesh_v6();
  }
}
module hotbed_foot_original_mesh_v6() {
  polyhedron(
    points = [
  ...
  translate(placement)
    scale(body_scale)
      union() {
        ...
        // Side/rear reinforcement is outside the original bounding box only.
        outside_wall_reinforcement(orig_length, orig_width, orig_height, wall_expansion_amount(), wall_expansion_area);

        // Notch Front reinforcement follows the source notch shape instead of using a rectangular bar.
        if (wall_expansion_area == "Sides and Notch Front" || wall_expansion_area == "Notch Front Only" || wall_expansion_area == "All Outside")
          notch_front_reinforcement(wall_expansion_amount(), notch_side, large_notch_width_scale(), large_notch_depth_scale());
      }
}
