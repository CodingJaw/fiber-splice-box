// Editable OpenSCAD conversion for hotbed_foot.stl.
// Self-contained: this file does not import or reference the source STL.
// v2 uses unique mesh vertices and centers the preview at the origin so it is visible when opened.
// Set stl_coordinates=true to place the model at the source STL coordinates for verification/export.
// Source mesh: 83 unique vertices, 166 triangular faces.

module hotbed_foot_v2(stl_coordinates = false) {
  source_center = [149.8822937, 138.25900269, 6];
  translate(stl_coordinates ? source_center : [0, 0, 0])
    polyhedron(
      points = [
