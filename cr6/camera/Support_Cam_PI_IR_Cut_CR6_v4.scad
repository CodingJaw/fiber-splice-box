// Support_Cam_PI_IR_Cut_CR6_v4.scad
...
// import the STL.  The source STL was used only to measure the overall bounding
// box, the diagonal arm direction, the camera tab location, and hole positions.

/* [Left Bracket Dimensions] */
base_z = 0;
base_thickness = 12;
diagonal_arm_width = 18;
diagonal_arm_start = [-8, 8];
diagonal_arm_end = [34, 46];
camera_tab_origin = [30, 40, 0];
camera_tab_size = [52, 20, 12];
camera_hole_xy = [64, 51];
camera_hole_diameter = 5.2;
end_block_center = [-8, 8];
end_block_size = [13.2, 13.2, 12];
end_block_rotation = 45;
...
module rotated_end_block() {
    translate([end_block_center[0], end_block_center[1], base_z])
        rotate([0, 0, end_block_rotation])
            translate([-end_block_size[0] / 2, -end_block_size[1] / 2, 0])
                rounded_rect(end_block_size, 1.2);
}

module left_bracket() {
    maybe_color()
        union() {
            // Continuous lower assembly: end block, diagonal arm, camera tab, and a small tower pad overlap.
            rotated_end_block();
