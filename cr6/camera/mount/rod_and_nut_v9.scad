...
fit_check_rotation = 0; // [0:1:359]
fit_check_z_offset = 0; // [-8:0.05:8]
...
nut_thread_engine = "BOSL2 matched nut"; // [BOSL2 matched nut, cutter subtract]
...
module threaded_nut_body() {
    // Prefer BOSL2 threaded_nut() for square/hex nuts so the female thread is generated
    // by the matching BOSL2 nut primitive instead of manually subtracting a cutter.
    if (nut_thread_engine == "BOSL2 matched nut" && (nut_outer_shape == "hex" || nut_outer_shape == "square")) {
        $fn = thread_resolution;
        threaded_nut(
            nutwidth=nut_width,
            id=[thread_minor_d, thread_mid_d, shaft_size],
            h=nut_height,
            pitch=active_pitch,
            shape=nut_outer_shape,
            left_handed=left_handed_thread,
            bevel=false,
            ibevel=false,
            blunt_start=false,
...
module fit_check_thread(length) {
    translate([0, 0, fit_check_z_offset])
        rotate([0, 0, fit_check_rotation])
            external_thread(length);
}

module sectioned_fit_check_thread(length) {
    if (show_section_cut == "yes") {
        difference() {
            fit_check_thread(length);
            translate([-nut_width, 0, -base_plate_thickness - 2])
                cube([nut_width * 2, nut_width, length + base_plate_thickness + 4]);
        }
    } else {
        fit_check_thread(length);
    }
...
module assembled_fit_check() {
    ...
    // When section cut is enabled, the bolt thread is cut by the same plane as the nut
    // so the two thread profiles can be compared directly.
    color([0.8, 0.8, 0.8, 0.55]) nut();
    color([0.1, 0.35, 1.0, 0.55]) sectioned_fit_check_thread(nut_height + base_plate_thickness);
}
...
module base_hole_interference_check() {
    ...
    color([1, 0, 0, 1])
        intersection() {
            base_plate_with_hole_and_countersink();
            translate([0, 0, -base_plate_thickness])
                fit_check_thread(base_plate_thickness);
        }
}
